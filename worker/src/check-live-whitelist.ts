...
try {
  ...
  // insert inactive -> no match yet.
  db.prepare(
    'INSERT INTO link_patterns (pattern, type, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?)',
  ).run('new.example.com', 'domain', 0, now(), now());
  assert.deepEqual(evaluateLinkGate(db, 'https://new.example.com/a.pdf'), []);

  // live activate mid-run -> matches on the very next message.
  db.prepare('UPDATE link_patterns SET active = 1, updated_at = ? WHERE pattern = ?').run(
    now(),
    'new.example.com',
  );
  assert.deepEqual(evaluateLinkGate(db, 'https://new.example.com/a.pdf'), [
