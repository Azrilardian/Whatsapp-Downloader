...
export type VtLookup = { status: 'clean' } | { status: 'flagged' } | { status: 'outage' };
...
// (AD-6 fail-closed), not swallowed here.
...
export function makeVtClient(apiKey: string): VtClient {
  return {
    async lookupHash(sha256: string): Promise<VtLookup> {
      ...
      try {
        response = await fetch(`${VT_FILES_API}/${sha256}`, { headers: { 'x-apikey': apiKey } });
      } catch {
        return { status: 'outage' };
      }
      if (response.status === 404) return { status: 'clean' };
      if (!response.ok) return { status: 'outage' };
